...
import com.yichen.statelayout.State.*
...
class StateLayout @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    init {
        ...
        useContentBgWhenLoading =
            ta.getBoolean(R.styleable.StateLayout_sl_useContentBgWhenLoading, StateLayoutConfig.useContentBgWhenLoading)
        enableLoadingShadow =
            ta.getBoolean(R.styleable.StateLayout_sl_enableLoadingShadow, StateLayoutConfig.enableLoadingShadow)
        enableTouchWhenLoading =
            ta.getBoolean(R.styleable.StateLayout_sl_enableTouchWhenLoading, StateLayoutConfig.enableTouchWhenLoading)
        defaultShowLoading =
            ta.getBoolean(R.styleable.StateLayout_sl_defaultShowLoading, StateLayoutConfig.defaultShowLoading)
    ...
    override fun onFinishInflate() {
        ...
        if (childCount > 0) {
            ...
            if (defaultShowLoading && !isInEditMode){
    ...
    fun showLoading(isForce: Boolean = false): StateLayout {
        if (showLoadingOnce && hasShowLoading && !isForce) return this
        mHandler.post {
    ...
    fun config(
        ...
        retryAction: ((errView: View) -> Unit)? = null
    ): StateLayout {
